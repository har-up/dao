...
## 基础
   ...
   - Handler
      ...
      - 内部原理
      	 Android消息机制由Handler、Lopper、MessageQueue三个对象运转。MessageQueue是一个消息队列（单链表），Looper负责不断的获取队列中的消息，交	      往每个消息的target属性即Handler对象处理。Looper中持有MessageQueue,在构造Handler时，有几个重载方式，有一个给定Looper对象的构造方法，最后	 都会把Looper中的队列引用给到Handler对象，这样Handler对象才能在sendMessage时添加消息到Looper对象。
   - AsyncTask
      封装了线程池和Handler的异步框架。
      - 使用
...
- volatile
  目前，计算机的cpu读写速度比内存的读写速度快几个数量级，所以引入了高速缓存/寄存器用于匹配到cpu速度从而获得更快的计算机操作体验，有了高速缓存/寄存器后，cpu读取数据的优先顺序是-> 寄存器 -> 高速缓存 -> 内存。原始数据存放在内存中，对于频繁读写的数据就会先缓存到寄存器和高速缓存中然后在合适的时机刷新内存中的值，此时就出现了多线程读写情况下的数据同步问题。volatile通过缓存一致性保证线程读取共享数据一定是最新值，因为每当volatile修饰的变量被修改时就会通知每个工作内存中持有该变量的线程该数据的缓存已失效，需要重新读入工作内存。
