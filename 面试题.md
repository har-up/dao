...
## OkHttp的使用及原理
   ...
   - 原理
      同步请求：当realCall.execute()时，通过client对象的dispatcher.execute()来执行请求。然后通过getResponseWithInterceptChain()来获取请求结果，其中是添加所有的拦截器，然后通过拦截器列表获取拦截链对象chain，通过chain.procced()来执行请求。
      异步请求：当realCall.enqueue()时，通过client对象的dispatcher.enqueue()来加入到请求队列中。请求队列分为三个：readyAsyncCalls（正在准备执行的异步请求队列）、runningAsyncCalls（正在执行的异步请求队列）、runningSyncCalls（正在运行中同步请求队列）。当正在执行的异步请求队列的大小小于限制的大小时就把请求加入到这个runningAsyncCalls队列并且通过executeService.execute(call)来执行该请求，否则加入到准备队列。executeService是一个ThreadPoolExecutor对象，其内部就是开启一个线程池执行队列中的请求（执行时也会有拦截器进行拦截处理）。
